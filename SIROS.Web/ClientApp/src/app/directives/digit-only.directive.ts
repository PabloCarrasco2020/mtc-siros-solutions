...
@Directive({ selector: '[appDigitOnly]' })
...
export class DigitOnlyDirective implements OnChanges {
  ...
  private isValidDecimal(sValue: string): boolean {
    if (!this.hasDecimalPoint) {
      return sValue.split(this.decimalSeparator).length <= 2;
    } else {
      ...
      if (selectedText && selectedText.indexOf(this.decimalSeparator) > -1) {
        ...
        return sValue.indexOf(this.decimalSeparator) < 0;
      }
    }
  }
